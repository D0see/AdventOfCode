...
const hasAWorkingPerm = (operation, result = [false], currNumIndex = 0, currPerm = undefined) => {
    if (result[0]) return;
...
const hasAWorkingPerm2 = (operation, result = [false], currNumIndex = 0, currPerm = undefined) => {
    if (result[0]) return;
    if (currNumIndex === operation.numbers.length)
        return (result[0] = currPerm === operation.result);
    hasAWorkingPerm2(
    ...
    hasAWorkingPerm2(
        ...
        currNumIndex + 1,
        parseInt(
            (currPerm === undefined ? "" : `${currPerm}`) + `${operation.numbers[currNumIndex]}`
        )
    );
    return result[0];
};
