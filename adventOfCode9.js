...
for (let i = 0; i < rawInput.length; i++) {
    ...
    for (let j = 0; j < parseInt(rawInput[i]); j++) litteralRep.push(Math.floor(i / 2));
}
...
while (right >= 0) {
    const spaceBounds = findFittingSpace(currBox[0], currBox[1] - currBox[0], litteralRep);
    if (spaceBounds) {
        fillSpace(spaceBounds, litteralRep, currBox[1] - currBox[0], litteralRep[currBox[0]]);
        eraseSlice(litteralRep, rightLeft, right);
    }
    rightLeft--;
    right = rightLeft;
    while (litteralRep[rightLeft - 1] === litteralRep[right] && rightLeft >= 0) rightLeft--;
