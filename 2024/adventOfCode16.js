...
const traverseBoard = (
  ...
  if (board[y][x] === "E" && (currResult < result[0] || !result[0]))
    result[0] = currResult;

  if (minScore[`${y}-${x}`] <= currResult) {
    return;
  }
  ...
  [
    ...
    if (board[newY][newX] !== "#")
      traverseBoard(
        ...
        newX,
        dir,
        currResult + (orientation === dir ? 1 : 1001),
        result,
        minScore
      );
  });
...
//PART 2
//recursive function to get to E
const traverseBoardAndGetBestSquares = (
  board,
  y,
  x,
  orientation,
  ...
  if (result[0] && currResult > result[0]) return;
  if (board[y][x] === "E") {
    if (!result[0] || currResult < result[0]) {
      result[0] = currResult;
  ...
  if (minScore[`${y}-${x}`] + 1000 < currResult) {
    return;
  }
  ...
  [
    ...
    if (board[newY][newX] !== "#")
      traverseBoardAndGetBestSquares(
        board,
        newY,
        newX,
        dir,
        ...
        bestSquares,
        minScore,
        JSON.parse(JSON.stringify(squareVisited))
      );
  });
};
