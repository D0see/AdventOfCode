...
const result = part1Matrix.flat().reduce((acc, cell) => (acc += cell === "X" ? 1 : 0), 0);
...
const blockingCauseLoop = (matrix, position) => {
    ...
    while (true) {
        orientationsAtPos[`${currGuardPos[0]}-${currGuardPos[1]}`]
            ? (orientationsAtPos[`${currGuardPos[0]}-${currGuardPos[1]}`][
                  [matrix[currGuardPos[0]][currGuardPos[1]]]
              ] = true)
            : (orientationsAtPos[`${currGuardPos[0]}-${currGuardPos[1]}`] = {
                  [matrix[currGuardPos[0]][currGuardPos[1]]]: true,
              });
        ...
        //Moves the guard to the next square
        if (
            orientationsAtPos[`${nextPos[0]}-${nextPos[1]}`]?.[
                matrix[currGuardPos[0]][currGuardPos[1]]
            ]
        )
            return true;
