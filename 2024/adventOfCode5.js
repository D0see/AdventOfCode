...
const rules = filteredByLines.slice(0, delimitationIndex).map((rule) => rule.split("|"));

const orders = filteredByLines.slice(delimitationIndex + 1).map((order) => order.split(","));

//key must be before value in order array
const rulesObj = rules.reduce((acc, rule) => {
    acc[rule[0]] ? (acc[rule[0]][rule[1]] = true) : (acc[rule[0]] = { [rule[1]]: true });
    return acc;
}, {});
...
invalidOrders
    .map((order) => fixOrder(rulesObj, order))
    .forEach((order) => {
